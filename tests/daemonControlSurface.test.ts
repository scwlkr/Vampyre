...
test("daemon control surface invokes the scheduler-selected review request once", async () => {
  ...
  try {
    ...
    assert.equal(first.projectId, "keepingus");
    assert.equal(first.issueUrl, "https://github.com/scwlkr/keepingus/issues/16");
    ...
    assert.equal(second.issueUrl, "https://github.com/scwlkr/keepingus/issues/16");
    assert.equal(reviewCalls, 1);
  } finally {
...
function reviewReport(workspaceRoot: string): ReviewRequestReport {
  return {
    ...
    github: {
      ...
      labelAction: "updated",
      issueNumber: 16,
      issueUrl: "https://github.com/scwlkr/keepingus/issues/16",
      issueAction: "reused",
      commentUrl: "https://github.com/scwlkr/keepingus/issues/16#issuecomment-1",
    },
    telegram: {
      status: "sent",
